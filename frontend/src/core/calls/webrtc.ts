...
export async function toggleScreenShare(userId: number): Promise<boolean> {
    ...
    if (!call.isScreenSharing) {
        ...
        try {
            ...
            // CRITICAL: Apply E2EE transform IMMEDIATELY before any frames are sent
            if (call.sessionKey && window.RTCRtpScriptTransform && !call.transformedSenders.has(sender)) {
                try {
                    const key = await importAesGcmKey(call.sessionKey);
                ...
                console.log("Session key exists:", !!call.sessionKey);
                console.log("RTCRtpScriptTransform available:", !!window.RTCRtpScriptTransform);
                console.log("Sender already transformed:", call.transformedSenders.has(sender));
            }

            // Configure screen share track for better browser window capture
            const constraints = videoTrack.getConstraints();
            console.log("Screen share track constraints:", constraints);
            
            // Apply additional settings for browser window stability
            await videoTrack.applyConstraints({
                width: { ideal: 1920 },
                height: { ideal: 1080 },
                frameRate: { ideal: 60, max: 60 }
            }).catch(e => {
                console.warn("Could not apply screen share constraints:", e);
            });

            // Browser window glitch prevention - force keyframe generation
            // This helps prevent the progressive glitching in browser windows
            let keyframeCounter = 0;
            const keyframeInterval = setInterval(async () => {
                if (videoTrack.readyState === 'live') {
                    try {
                        // Force a keyframe every 5 seconds to prevent glitch accumulation
                        await videoTrack.applyConstraints({
                            frameRate: { ideal: 30, max: 30 }
                        });
                        keyframeCounter++;
                        console.log(`Screen share keyframe forced #${keyframeCounter}`);
                    } catch (e) {
                        console.warn("Could not force keyframe:", e);
                    }
                } else {
