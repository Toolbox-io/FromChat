...
export class DMPanel extends MessagePanel {
    ...
    private async parseTextPayload(env: DmEnvelope, decryptedMessages: Message[]) {
        const plaintext = await decryptDm(env, new Uint8Array([this.dmData!.recipientId]));
        const isAuthor = env.senderId !== this.dmData!.recipientId;
        const username = isAuthor ? this.currentUser.currentUser?.username ?? "You" : this.dmData!.recipientUsername;
    ...
    async loadMessages(): Promise<void> {
        ...
        try {
            const messages = await fetchDMHistory(this.dmData.recipientId, this.currentUser.authToken);
            ...
            for (const env of messages) {
                try {
                    ...
                    if (env.senderId === this.dmData!.recipientId && env.id > maxIncomingId) {
            ...
            // Update last read ID
            if (maxIncomingId > 0) {
                await this.setLastReadId(this.dmData.recipientId, maxIncomingId);
            }
    ...
    setDMData(dmData: DMPanelData): void {
        ...
        this.updateState({
            id: `dm-${dmData.recipientId}`,
            title: dmData.recipientUsername,
    ...
    handleWebSocketMessage = async (response: DMWebSocketMessage): Promise<void> => {
        if (response.type === "dmNew" && this.dmData) {
            ...
            if (envelope.senderId === this.dmData.recipientId || envelope.recipientId === this.dmData.recipientId) {
                try {
                    const dmMsg = await this.parseTextPayload(envelope, this.getMessages());
                    
                    // Check if this is a confirmation of a message we sent
                    const isOurMessage = envelope.senderId !== this.dmData.recipientId;
                    if (isOurMessage) {
                    ...
                    if (envelope.senderId === this.dmData.recipientId) {
                        const currentLastRead = await this.getLastReadId(this.dmData.recipientId);
                        await this.setLastReadId(this.dmData.recipientId, Math.max(currentLastRead, envelope.id));
                    }
                } catch (error) {
        ...
        if (response.type === "dmEdited" && this.dmData) {
            ...
            try {
                ...
                const plaintext = await decryptDm(
                    ...
                    new Uint8Array([this.dmData.recipientId])
                );
                let content = plaintext;
    ...
    private async getLastReadId(userId: number): Promise<number> {
        try {
            const v = await AsyncStorage.getItem(`dmLastRead:${userId}`);
            return v ? Number(v) : 0;
        } catch {
            return 0;
        }
    ...
    private async setLastReadId(userId: number, id: number): Promise<void> {
        try {
            await AsyncStorage.setItem(`dmLastRead:${userId}`, String(id));
        } catch {}
    }
    ...
    async handleDeleteMessage(messageId: number): Promise<void> {
        ...
        await deleteDmEnvelope(messageId, this.currentUser.authToken);
    }
    ...
    async handleEditMessage(messageId: number, content: string): Promise<void> {
        ...
        editDmEnvelope(messageId, JSON.stringify(payload), this.currentUser.authToken).catch((e) => {
