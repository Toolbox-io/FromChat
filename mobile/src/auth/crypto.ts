...
async function saveKeys(
	...
	await SecureStore.setItemAsync("publicKey", encodedPublicKey);
	await SecureStore.setItemAsync("privateKey", encodedPrivateKey);
}
...
export async function restoreKeys() {
	try {
		const publicKeyStr = await SecureStore.getItemAsync("publicKey");
		const privateKeyStr = await SecureStore.getItemAsync("privateKey");
		
		if (publicKeyStr && privateKeyStr) {
			currentPublicKey = ub64(publicKeyStr);
			currentPrivateKey = ub64(privateKeyStr);
		}
	} catch (error) {
