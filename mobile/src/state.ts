...
export interface UserState {
    ...
    uploadProfilePictureData: (file: File) => void;
}
...
interface AppState {
    ...
    switchToDm: (userId: number) => Promise<void>;
...
export const useAppState = create<AppState>()(
  persist(
    ...
    user: {
        ...
        uploadProfilePictureData: (file: File) => {}
    },
    ...
    restoreUserFromStorage: async () => {
        ...
        // Mobile doesn't need electron receiver
                            }
                        }
                    } catch (e) {
    ...
    switchToDM: async (dmData: DMPanelData) => {
        ...
        set((state) => ({
            chat: {
                ...
                dmPanel: dmPanel,
                activeDm: {
                    userId: dmData.recipientId,
                    username: dmData.recipientUsername,
                    publicKey: ""
                },
                activeTab: "dms"
            }
    ...
    switchToDm: async (userId: number) => {
        const state = get();
        const { user, chat } = state;
        ...
        // Find user data
        const dmUser = chat.dmUsers.find(u => u.id === userId);
        if (!dmUser) return;
        
        // Create DM panel data
        const dmData: DMPanelData = {
            recipientId: userId,
